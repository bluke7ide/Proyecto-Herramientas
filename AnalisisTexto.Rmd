...
```{r}
a <- sapply(telegram$mensaje, function(x) typeof(x) == "list")
b <- sapply(telegram$mensaje[a], function(x) data.frame(x))
c <- sapply(b, function(x) x %>% select(-type))
d <- data.frame(sapply(c, function(x) paste(x, collapse = "")))
```
