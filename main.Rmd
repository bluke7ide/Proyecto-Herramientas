...
En primer lugar, se procede a extraer patrones presentes en la conversación 
...
repetir una y otra vez la traducción, esta se guardará en un ".csv" y luego se 
trabajará únicamente con dicho archivo.

```{python}
gen_py.df = gen_py.df.head(10) # borrar esta línea al final
...
Una vez realizado el análisis de sentimientos, se puede comenzar a extraer 
resultados.
...
De manera análoga, se procede a extraer patrones presentes en la conversación 
utilizando los métodos creados.
...
```{python}
herra_py.df = gen_py.df.head(10) # borrar esta línea al final

herra_py.traducir()
herra_py.guardar("herraII_08-07-24")
```
...
```{python}
herra_py.leer("herraII_08-07-24")
...
```{python}
herra_py.sentimiento_predominante('Luis Fer Amey')
